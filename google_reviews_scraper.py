...
class GoogleReviewsScraper:
        ...
        if self.driver.current_url.startswith("https://consent.google.com/"):
            WebDriverWait(self.driver, 5).until(
                expected_conditions.element_to_be_clickable(
                    (By.CSS_SELECTOR, ".VtwTSb form:first-of-type button")
                )
            ).click()
    ...
    def check_reviews_link_clickable(self):
        def check_condition(self):
            if (
                len(
                    self.find_elements(
                        By.CSS_SELECTOR, ".LBgpqf .dmRWX .mmu3tf span:last-of-type"
                    )
                )
                > 0
            ):
                ...
                len(
                    self.find_elements(
                        By.CSS_SELECTOR, ".kA9KIf .PPCwl .jANrlb .HHrUdb"
                    )
                )
                > 0
            ):
    ...
    def scroll_to_bottom(self):
        WebDriverWait(self.driver, 5).until(
            expected_conditions.element_to_be_clickable(
                (By.CSS_SELECTOR, ".aIFcqe  .WNBkOb .dS8AEf")
            )
        )
        scrollbar = self.driver.find_element(
            By.CSS_SELECTOR, ".aIFcqe  .WNBkOb .dS8AEf"
        )
        total_number_of_reviews_text = self.driver.find_element(
            By.CSS_SELECTOR, ".PPCwl .Bd93Zb .jANrlb .fontBodySmall"
        ).get_attribute("innerText")
        total_number_of_reviews = int(total_number_of_reviews_text.split()[0])
        nr_of_loaded_reviews = len(
            self.driver.find_elements(
                By.CSS_SELECTOR, ".aIFcqe  .WNBkOb .dS8AEf .m6QErb .jftiEf"
            )
        )
        while True:
            ...
            self.driver.execute_script(
                "arguments[0].scrollTop = arguments[0].scrollHeight", scrollbar
            )
            if nr_of_loaded_reviews != len(
                self.driver.find_elements(
                    By.CSS_SELECTOR, ".aIFcqe  .WNBkOb .dS8AEf .m6QErb .jftiEf"
                )
            ):
                ...
                print(
                    f"{nr_of_loaded_reviews} of {total_number_of_reviews} reviews have been loaded."
                )
    ...
    def run(self):
        try:
            ...
            print(
                "2.Google either changed the layout of the page or the CSS class names."
            )
            print("3.Google changed the design of the API response.")
            print("4.Network related issues.")
            print(
                "Tip: comment out options.add_argument('headless') to see what happens in the browser."
            )
